        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="row">
              ...
              <div class="col-md-3">
                <b-select name="fund_distributor_id" :options="distributions" :value="investorSipDetail.fund_distributor_id" :disabled="true" />
              </div>
            </div>
            <div class="row">
              <label class="col-md-3 col-form-label">{{ $t('investor_sip.Trading account') }}</label>
              <div class="col-md-3">
                <b-select name="investor_id" :options="refsTrandingAccount" :value="investorSipDetail.investor_id" :disabled="true" />
              </div>
            </div>
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <b-select name="fund_certificate_id" :options="fund_certificate_id" :value="investorSipDetail.fund_certificate_id" :disabled="true" />
              </div>
            </div>
            <div v-if="fund_product_id !== null" class="row">
              <label class="col-md-3 col-form-label">{{ $t('investor_sip.SIP Product') }}</label>
              <div class="col-md-3">
                <b-select name="fund_product_id" :options="fund_product_id" :value="investorSipDetail.fund_product_id" @input="onChangeSipProduct" :disabled="true" />
              </div>
            </div>
            <div class="row">
              <label class="col-md-3 col-form-label">{{ $t('investor_sip.Payment method') }}</label>
              <div class="col-md-3">
                <b-select name="payment_type" :options="payment_type" :value="investorSipDetail.payment_type" />
              </div>
            </div>
            <div class="row">
              ...
              <div class="col-md-3">
                <b-input
                  ...
                  :value="investorSipDetail.periodic_amount"
                  type="text"
                />
              </div>
            </div>
          ...
          <div class="card-footer">
            <div class="row">
              ...
              <div class="col-md-8">
                <button type="submit" class="btn btn-primary">{{ $t('investor_sip.Update') }}</button>
              </div>
            </div>
          </div>
        </div>
...
import { callApiGetSipType, callApiInvestorSipUpdate, callApiInvestorSipDetail } from '@/apis/investor_sip.js'
...
export default {
  ...
  async created () {
    ...
    this.handleGetInvestorSipDetail()
  },
  methods: {
    ...
    async onSubmit (params) {
      ...
      const result = await callApiInvestorSipUpdate(this, params)
      if (result.status === true) {
        this.flashMessage.success({
          title: this.$t('investor_sip.Update Success'),
          message: this.$t('investor_sip.Update Sip success')
        })
        this.$router.push(this.localePath('/investor-sip'))
      } else {
        this.flashMessage.error({
          title: this.$t('investor_sip.Update Error'),
          message: result.message
        })
      }
    },
    async handleGetInvestorSipDetail () {
      const data = await callApiInvestorSipDetail(this, { investor_sip_id: this.$route.params.id })
      if (data.status === true) {
        this.investorSipDetail = data.response
        this.handleGetTradingAccountNumber(this.investorSipDetail.fund_distributor_id)
        this.handleGetDetailAndSipProduct(this.investorSipDetail.investor_id)
        this.handleGetSipProduct(this.investorSipDetail.fund_certificate_id)
        this.handGetSipPacKageInfo(this.investorSipDetail.fund_product_id)
      } else {
