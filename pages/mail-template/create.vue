<template>
  <div class="container">
    ...
    <div class="row">
      <label class="col-form-label col-md-2">{{ $t('mail_template.select mail template') }}</label>
      <div class="col-md-8">
        <multiselect
          v-model="value"
...
import { callApiMailTemplateLocaleCreate } from '@/apis/mail_template_locale'
  ...
  methods: {
    async saveContent () {
      ...
      if (flag) {
        ...
        const result = await callApiMailTemplateLocaleCreate(this, params)
        if (result.status) {
